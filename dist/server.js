...
var loginUserIntoDB = async (payload) => {
  ...
  const refreshToken2 = jwt2.sign(jwtpayload, config_default.refresh_token, { expiresIn: config_default.expiresIn });
...
var generateRefreshToken = async (token) => {
  ...
  ;
  const decoded = jwt2.verify(token, config_default.refresh_token);
  const userData = await pool.query(`
            SELECT * FROM users WHERE email = $1
            `, [decoded.email]);
  const user = userData.rows[0];
